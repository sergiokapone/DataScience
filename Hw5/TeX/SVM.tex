...
Ось ми розібрались, як побудувати лінійний класифікатор за допомогою методу опорних векторів. Проте, як ми знаємо, часто ми не можемо лінійно розділити наші дані, як наприклад на рис.~\ref{fig:inseparable}.
...
\begin{figure}[h!]\centering
	\subcaptionbox{Не роздільні ознаки в 2D просторі\label{fig:inseparable}}{
    ...
    \subcaptionbox{Додавання ознаки $x_3$}{
    	\begin{tikzpicture}[>=latex, scale=0.95]
        ...
        text=white,
        ] at (-2.5,2.5, 0) {$\phi(x_1, x_2) = (x_1, x_2, x_3)$};

        % Класс 1 - внутренний круг
        \foreach \i in {1,...,100} {
            \pgfmathsetmacro{\x}{1.5 + 2*rnd}
            \pgfmathsetmacro{\y}{1.5 + 2*rnd}
            \pgfmathsetmacro{\z}{1.5 + 2*rnd}
            \fill[seaborn_blue] (\x, \y, \z) circle (2pt);
        }
        ...
        \foreach \i in {1,...,150} {
            ...
            \pgfmathsetmacro{\z}{3 + \radius*sin(\angle)}
            \fill[seaborn_red] (\x, \y, \z) circle (2pt);
        }

    		% Оси координат
    		\draw[->] (0, 0, 0) -- ++(5,0,0) node[below] {$x_1$};
    		\draw[->] (0, 0, 0) -- ++(0,5,0) node[left]  {$x_2$};
            \draw[->] (0, 0, 0) -- ++(0,0,6) node[left]  {$x_3$};
    	\end{tikzpicture}
        }
        \caption{Ілюстрація ядрового трюку\label{fig:kernel}}
\end{figure}
